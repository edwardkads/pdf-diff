...
class EqualDocumentsException(Exception):
    pass
...
def render_changes(changes, styles, width):
    ...
    if len(changes) == 0:
        raise EqualDocumentsException("There are no text differences.")
...
def main():
    ...
    parser.add_argument('-f', '--format', choices=['png', 'gif', 'jpeg', 'ppm', 'tiff', 'pdf'], default='png',
    ...
    if args.changes:
        ...
        try:
            img = render_changes(json.load(sys.stdin),
                                 style, args.result_width)
        except EqualDocumentsException:
            sys.stderr.write(str({'status': 'equal'}))
            sys.exit(1)
        if args.format.upper() == 'PDF':
            img.convert('RGB').save(sys.stdout.buffer,
                                    args.format.upper(), save_all=True)
        else:
            img.save(sys.stdout.buffer, args.format.upper())
        sys.stderr.write(str({'status': 'different'}))
